...
{
	unsigned int index = blockgroup_for_inode(inode_number);
	unsigned int offset = blockgroup_inode_index(inode_number);

	Ext2FSBlockGroupDescriptor* grupo = block_group(index);

	unsigned int pos_inodo = grupo->inode_table + (sizeof(Ext2FSInode) * offset);

	Ext2FSInode* inodo = (Ext2FSInode*) malloc(sizeof(Ext2FSInode));
	read_block(pos_inodo, (unsigned char*)inodo);	// copiamos el inodo, tiene tamaño de un bloque
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	if (block_number < 256) {
		...
		unsigned int size = block_size / sizeof(int);
		unsigned int offset = block_number % size;
		read_block(((unsigned int*)buffer)[offset], buffer);

		block_number -= (offset * size);
		res = ((unsigned int*) buffer)[block_number];
	}

	// 3er indireccion no es necesaria
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	//TODO: Ejercicio 3
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned char blocks[block_size*2];
	unsigned int index_bloque = 0;
